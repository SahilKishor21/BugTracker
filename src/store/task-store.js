...
export const useTaskStore = create((set, get) => ({
  ...
  getFilteredTasks: (userRole, userId) => {
    ...
    let filteredTasks = tasks.filter((task) => {
      if (userRole === "developer") {
        console.log("Checking task for developer:", {
          taskId: task.id,
  ...
  getTaskStats: (userRole, userId) => {
    ...
    return {
      ...
      pendingApproval: userTasks.filter(
      ...
      totalTimeSpent: userTasks.reduce(
  ...
  getChartData: (userRole, userId) => {
    ...
    for (let i = 6; i >= 0; i--) {
      ...
      if (i === 6) {
        ...
        dayData.inProgress = Math.min(
          1,
          userTasks.filter((t) => t.status === "in-progress").length
        );
        dayData.completed = Math.min(
          1,
